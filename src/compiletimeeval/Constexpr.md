...
4. **Can include control structures and constructs,** such as `if`, `switch`, `for`, `while`, and `do-while` loops, provided they don't violate other constexpr constraints. `static_assert`, `typedef`, `using`, `if constexpr`, and `return`are also allowed:
...
5. **Only call other `constexpr` functions:**
